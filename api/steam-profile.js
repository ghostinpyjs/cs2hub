...
export default async function handler(req, res) {
  ...
  const favWeapon = (() => {
    ...
    for (const w of weapons) { const k = getStat(`total_kills_${w}`); if (k > best.kills) best = { name: w.toUpperCase(), kills: k }; }
    return best.name;
  })();
  ...
  const hoursRes = await fetch(`https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key=${key}&steamid=${steamid}&appids_filter[0]=730&include_appinfo=false`);
  const hoursData = await hoursRes.json();
  const hours = Math.round((hoursData?.response?.games?.[0]?.playtime_forever ?? 0) / 60);

  const result = {
    steam_id: steamid, nick: player.personaname, avatar: player.avatarfull,
    profile_url: player.profileurl, steam_level: steamLevel, kills, deaths,
    kd, hs_percent: hsPercent, mvps, wins, fav_weapon: favWeapon, hours, elo: 0,
