...
function StatsTab({ steamid }) {
  ...
  if (err) return <div><Err msg={err} /><Empty icon="📊" title="Stats Indisponíveis" text="Esse perfil pode estar privado. Verifique as configurações de privacidade no Steam." /></div>;
  ...
  return (
    <div className="stats-grid">
      ...
      <div className="stat-card">
        ...
        <div className="stat-card-val">{fmt(s.kills)}</div>
        <div className="stat-card-lbl">Kills</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">☠️</div>
        <div className="stat-card-val">{fmt(s.deaths)}</div>
        <div className="stat-card-lbl">Deaths</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">🎯</div>
        <div className="stat-card-val">{s.hsP}%</div>
        <div className="stat-card-lbl">Headshot %</div>
      ...
      <div className="stat-card">
        <div className="stat-card-icon">🔫</div>
        <div className="stat-card-val">{s.acc}%</div>
        <div className="stat-card-lbl">Precisão</div>
      ...
      <div className="stat-card">
        <div className="stat-card-icon">🏆</div>
        <div className="stat-card-val">{fmt(s.wins)}</div>
        <div className="stat-card-lbl">Vitórias</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">📈</div>
        <div className="stat-card-val">{s.wr}%</div>
      ...
      <div className="stat-card">
        <div className="stat-card-icon">🎮</div>
        <div className="stat-card-val">{fmt(s.matches)}</div>
        <div className="stat-card-lbl">Partidas</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">🌀</div>
        <div className="stat-card-val">{fmt(s.rounds)}</div>
        <div className="stat-card-lbl">Rounds</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">⭐</div>
        <div className="stat-card-val">{fmt(s.mvps)}</div>
        <div className="stat-card-lbl">MVPs</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">💥</div>
        <div className="stat-card-val">{fmt(s.damage)}</div>
        <div className="stat-card-lbl">Dano Total</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">🎯</div>
        <div className="stat-card-val">{fmt(s.awpK)}</div>
        <div className="stat-card-lbl">Kills AWP</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">🔪</div>
        <div className="stat-card-val">{fmt(s.knifeK)}</div>
        <div className="stat-card-lbl">Kills Faca</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">💣</div>
        <div className="stat-card-val">{fmt(s.bp)}</div>
        <div className="stat-card-lbl">Bombas Plantadas</div>
      </div>
      <div className="stat-card">
        <div className="stat-card-icon">🛡️</div>
        <div className="stat-card-val">{fmt(s.bd)}</div>
        <div className="stat-card-lbl">Bombas Desarmadas</div>
      </div>
    </div>
  );
}
...
function FeedTab({ currentPlayer, currentSteamid }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/friends?steamid=${currentSteamid}`)
      .then(r => r.json())
    ...
    <div className="community-layout">
      ...
      <div>
        ...
        <div className="feed-compose">
          ...
          <textarea
            ...
            placeholder="Compartilhe uma conquista, peça dicas, ou poste qualquer coisa sobre CS2..."
            value={content}
          ...
          <div className="compose-footer">
            ...
            <button className="compose-btn" onClick={submitPost} disabled={posting || !content.trim() || content.length > MAX}>
            ...
            <div key={post.id} className="post-card" style={{ animationDelay: `${i * 0.04}s` }}>
              <div className="post-header">
                {post.avatar
                  ...
                  : <div className="post-avatar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg4)', fontSize: 18 }}>👤</div>
                }
                <div>
              ...
              <div className="post-footer">
                <button className={`like-btn ${post.likedBy?.includes(sid) ? 'liked' : ''}`} onClick={() => toggleLike(post.id)}>
      ...
      <div className="sidebar">
                ...
                return (
                  <a key={f.steamid} href={f.profileurl} target="_blank" rel="noopener noreferrer" className="sidebar-item">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="sidebar-item-name">{f.username}</div>
                      <div className="sidebar-item-sub">{f.gameextrainfo || st.label}</div>
                    </div>
        ...
        <div className="sidebar-card">
          ...
          <div style={{ padding: '14px 18px', fontSize: 13, color: 'var(--text2)', lineHeight: 1.7 }}>
            Plataforma de stats e comunidade para jogadores de CS2. Carregue seu perfil Steam para ver suas estatísticas, inventário e interagir com outros jogadores.
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      ...
      {sorted.length === 0 ? (
        <div className="rank-empty">
          ...
          <div className="rank-empty-txt">O ranking será preenchido conforme os jogadores carregarem seus perfis. Carregue o seu para entrar!</div>
        </div>
      ) : (
    ...
    <div className="profile-search-wrap">
          ...
          <div className="found-info">
            ...
            <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
              <div className={`profile-status ${st?.cls}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                <span className="s-dot" style={{ width: 6, height: 6, borderRadius: '50%', background: 'currentColor' }} />
                {st?.label}
              </div>
      ...
      {!result && !loading && !err && (
        <div style={{ marginTop: 32, padding: '24px', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', fontSize: 14, color: 'var(--text3)', lineHeight: 1.8 }}>
          ...
          Você pode buscar pelo nome de usuário (ex: <b style={{ color: 'var(--text)' }}>valve</b>), pela URL personalizada (ex: <b style={{ color: 'var(--text)' }}>gaben</b>), ou pelo SteamID64 (ex: <b style={{ color: 'var(--text)' }}>76561197960287930</b>).
        </div>
      )}
    </div>
  );
...
const PAGES = ['meu-perfil', 'feed', 'ranking', 'perfis'];
...
export default function Home() {
              ...
              if (p) {
                fetch('/api/ranking', {
                  ...
                  body: JSON.stringify({
                    ...
                    kills: s.kills, deaths: s.deaths, wins: s.wins,
                    kdr: s.kdr, hsPercent: s.hsP,
                  }),
                }).catch(() => {});
              }
            });
      ...
      if (playtimeRes.status === 'fulfilled') {
        ...
        if (g) {
          ...
          fetch('/api/ranking', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ steamid: sid, username: playerRes.value?.response?.players?.[0]?.personaname || '', avatar: playerRes.value?.response?.players?.[0]?.avatarmedium || null, hours: hrs }),
          }).catch(() => {});
        }
      }
    } catch {
  ...
  const handleLoadFromRanking = (sid) => {
    loadProfile(sid);
    setPage('meu-perfil');
  };
  ...
  return (
    <>
      ...
      <main className="main">
        {/* ── Hero (only on perfil page when no player) ── */}
        {page === 'meu-perfil' && !player && !loading && (
          <div className="hero">
            ...
            <div className="hero-search">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: 'var(--text3)', flexShrink: 0 }}>
                <circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" />
              </svg>
              <input
                placeholder="Seu nome de usuário Steam ou SteamID64..."
                value={input}
                onChange={e => setInput(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && loadProfile()}
                autoFocus
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 32, flexWrap: 'wrap' }}>
              {[['feed', '💬', 'Feed'], ['ranking', '🏆', 'Ranking'], ['perfis', '🔍', 'Perfis']].map(([id, icon, label]) => (
                <button key={id} onClick={() => setPage(id)} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--text2)', borderRadius: 'var(--radius-sm)', padding: '8px 20px', fontFamily: 'var(--font)', fontSize: 14, cursor: 'pointer', transition: 'all .2s' }}
