...
export default async function handler(req, res) {
  // req.query tem as chaves com ponto (ex: 'openid.mode') — precisamos
  // serializar manualmente para não re-encodar o que a Steam já enviou.
  const params = Object.entries(req.query)
    .map(([k, v]) => `${encodeURIComponent(k)}=${encodeURIComponent(v)}`)
    .join('&');

  // Troca o mode para check_authentication
