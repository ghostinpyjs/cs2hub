        ...
        <div className="modal-body">
          ...
          <div className="modal-img-area">
            {item.icon_url
              ...
              : <div style={{ fontSize: 56, color: 'var(--text3)' }}>🔫</div>}
          </div>
  ...
  return (
    <div className="stats-grid">
      ...
      {[
        ...
        ['💣', fmt(s.bp),      'Bombas Plantadas'],
        ['🛡️', fmt(s.bd),     'Bombas Desarmadas'],
      ].map(([icon, val, lbl]) => (
        ...
        : (
          <div className="inv-grid">
            {filtered.map((item, i) => {
              const rc = rarityColor(item.rarity, item.rarity_color);
              return (
                <div key={`${item.assetid}-${i}`} className="item-card"
                  style={{ animationDelay: `${Math.min(i * 0.025, 0.4)}s`, borderColor: `${rc}30` }}
      ...
      <div className="sidebar">
        {currentPlayer && (
          <div className="sidebar-card">
            <div className="sidebar-title">
              👥 Amigos
              {onlineCount > 0 && <span style={{ fontSize:12, color:'var(--green)', fontFamily:'var(--font-mono)' }}>{onlineCount} online</span>}
            </div>
            {friends.length === 0
        ...
        <div className="sidebar-card">
          ...
          <div style={{ padding:'14px 18px', fontSize:13, color:'var(--text2)', lineHeight:1.7 }}>
            Plataforma de stats e comunidade para jogadores de CS2. Faça login com Steam ou carregue qualquer perfil para ver estatísticas, inventário e interagir com a comunidade.
          </div>
        </div>
      </div>
    </div>
        ...
        : (
          <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'var(--radius)', overflow:'hidden' }}>
            <table className="ranking-table">
              <thead>
                <tr><th>#</th><th>Jogador</th><th>Kills</th><th>K/D</th><th>Vitórias</th><th>HS%</th><th>Horas</th></tr>
              </thead>
              <tbody>
                {sorted.map((p, i) => (
                  <tr key={p.steamid}>
                    ...
                    <td>
                      <div className="rank-player">
                        {p.avatar
                          ...
                          : <div className="rank-av" style={{ display:'flex', alignItems:'center', justifyContent:'center', background:'var(--bg4)', fontSize:14 }}>👤</div>
                        }
  ...
  const search = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const pRes  = await fetch(`/api/player?steamid=${data.results[0].steamid}`);
        const pData = await pRes.json();
        const p = pData?.response?.players?.[0];
        setResult(p ? { ...p } : data.results[0]);
      } else if (data.results?.length > 1) {
  ...
  return (
    <div className="profile-search-wrap">
      <div style={{ marginBottom:24 }}>
        ...
        <div style={{ fontSize:14, color:'var(--text2)' }}>Digite uma URL personalizada do Steam ou SteamID64 para encontrar qualquer jogador.</div>
      </div>
      ...
      {results.length > 1 && (
        <div className="search-results-list">
          {results.map(r => (
            <button key={r.steamid} className="search-result-item" onClick={() => onLoadProfile(r.steamid)}>
              {r.avatar && <img src={r.avatar} alt={r.username} className="search-result-av" />}
              <div className="search-result-info">
                <div className="search-result-name">{r.username}</div>
                <div className="search-result-sid">{r.steamid}</div>
              </div>
      ...
      {result && (
        <div className="found-profile-card">
          ...
          <div className="found-info">
            <div className="found-name">{result.personaname || result.username}</div>
            <div className="found-sid">{result.steamid}</div>
            {st && (
              <div className={`profile-status ${st.cls}`} style={{ marginTop:8, display:'inline-flex', alignItems:'center', gap:5, padding:'3px 10px', borderRadius:20, fontSize:12, fontWeight:600 }}>
                <span className="s-dot" style={{ width:6, height:6, borderRadius:'50%', background:'currentColor' }} />{st.label}
              </div>
            )}
            {result.gameextrainfo && <span style={{ fontSize:13, color:'var(--blue)', marginTop:6, display:'block' }}>🎮 {result.gameextrainfo}</span>}
          </div>
      ...
      {!result && results.length === 0 && !loading && !err && (
        <div style={{ marginTop:32, padding:'24px', background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:'var(--radius)', fontSize:14, color:'var(--text3)', lineHeight:1.8 }}>
          ...
          Busque pela URL personalizada (ex: <b style={{ color:'var(--text)' }}>gaben</b>) ou pelo SteamID64 (ex: <b style={{ color:'var(--text)' }}>76561197960287930</b>).
        </div>
      )}
    </div>
  );
...
export default function Home({ steamUser, onLogout }) {
  ...
  // ── Auto-load quando usuário faz login via Steam OAuth ──────────────────────
  useEffect(() => {
    if (steamUser?.steamid && !player && !loading) {
      ...
      if (playtimeRes.status === 'fulfilled') {
        ...
        if (g) {
          ...
          fetch('/api/ranking', {
            ...
            body: JSON.stringify({
              steamid: sid,
              username: playerRes.value?.response?.players?.[0]?.personaname || '',
              avatar:   playerRes.value?.response?.players?.[0]?.avatarmedium || null,
              hours: hrs,
            }),
          }).catch(() => {});
        }
    ...
    <>
      ...
      <header className="header">
        <div className="header-inner">
          ...
          {/* SteamAuth — login Steam + busca por username com dropdown */}
          ...
          <div className="hero">
            ...
            <div className="hero-cta-group">
              ...
              <div className="hero-search">
                ...
                <input
                  id="hero-input"
                  placeholder="URL personalizada ou SteamID64..."
                  onKeyDown={e => e.key === 'Enter' && handleUserFound(e.target.value)}
                />
                <button className="hero-search-btn" onClick={() => {
                  const v = document.getElementById('hero-input')?.value;
                  if (v) handleUserFound(v);
                }}>Buscar</button>
              </div>
            </div>
            ...
            <div className="hero-quick">
              {[['feed','💬','Feed'],['ranking','🏆','Ranking'],['perfis','🔍','Perfis']].map(([id,icon,label]) => (
                <button key={id} onClick={() => setPage(id)} className="hero-quick-btn">{icon} {label}</button>
              ))}
            </div>
          </div>
        )}
